...
export default function Login() {
  ...
  return (
    <div className="relative min-h-[calc(100vh-64px)] dot-grid overflow-hidden flex items-center justify-center px-6">
      <Blob accent="violet" variant={1} className="w-56 h-56 -top-16 -left-16 opacity-60" />
      <Blob accent="amber" variant={3} className="w-32 h-32 bottom-10 right-10 opacity-60 hidden sm:block" />
      ...
      <div className="sticker relative max-w-sm w-full p-6 rounded-2xl border-2 border-slate-900 dark:border-slate-950 bg-white dark:bg-slate-800">
        ...
        <p className="mt-4 text-sm text-slate-500 dark:text-slate-400">
          No account?{' '}
          <Link to="/signup" className="text-violet-600 dark:text-violet-400 underline font-medium">
            Sign up
          </Link>
        </p>
      </div>
